...
export default function Navigation({ dictionary, locale }: NavigationProps) {
  ...
  const handleSportsClick = (e: React.MouseEvent<HTMLAnchorElement>) => {
  ...
  return (
    <nav className="sticky top-0 z-40 bg-white/95 backdrop-blur-sm border-b border-semantic-border-DEFAULT">
      <div className="max-w-7xl mx-auto px-5 sm:px-4 relative">
              ...
              {navItems.map((item) => (
                <li key={item.href}>
                  <Link
                    href={item.href}
                    className={cn(
                      "text-semantic-text-primary font-medium py-2 relative transition-colors hover:text-semantic-brand-default",
                      item.id === 'home' && pathname === `/${locale}` && [
                        "text-semantic-brand-default",
                        "after:content-[''] after:absolute after:bottom-[-1px] after:left-0 after:right-0 after:h-[2px] after:bg-semantic-brand-default"
                      ]
                    )}
                  >
                    {item.label}
...
<button
            className="lg:hidden ml-auto p-2 rounded-md hover:bg-semantic-background-secondary transition-colors"
            onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
            aria-label="Toggle menu"
          >
            {isMobileMenuOpen ? (
              <X className="h-6 w-6 text-semantic-text-primary" />
            ) : (
              <Menu className="h-6 w-6 text-semantic-text-primary" />
            )}
          </button>
        </div>
<div className={cn(
          "lg:hidden absolute top-full left-0 right-0 bg-white border-b border-semantic-border-DEFAULT shadow-lg",
          "transition-all duration-300 overflow-hidden",
          isMobileMenuOpen ? "max-h-[600px] opacity-100" : "max-h-0 opacity-0 pointer-events-none"
        )}>
          <ul className="py-4 px-5 sm:px-4 space-y-2">
            {navItems.map((item) => (
              <li key={item.href}>
                <Link
                  href={item.href}
                  className={cn(
                    "block px-4 py-3 rounded-md transition-colors hover:bg-semantic-background-secondary",
                    "text-semantic-text-primary font-medium",
                    item.id === 'home' && pathname === `/${locale}` && "bg-semantic-brand-default/10 text-semantic-brand-default"
                  )}
                  onClick={() => {
          ...
          <div className="px-4 pb-4 border-t border-semantic-border-subtle pt-4">
            <LanguageSwitcher currentLocale={locale} />
          </div>
        </div>
      </div>
